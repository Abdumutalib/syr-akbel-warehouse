...
const DEFAULT_STATE_PATH = path.join(__dirname, '../data/warehouse.json');
const configuredStatePath = String(process.env.WAREHOUSE_STATE_FILE || '').trim();
const STATE_PATH = configuredStatePath
  ? (path.isAbsolute(configuredStatePath)
      ? configuredStatePath
      : path.resolve(process.cwd(), configuredStatePath))
  : DEFAULT_STATE_PATH;
...
function safeWriteState(state) {
  ...
  const tempPath = path.join(stateDir, `${path.basename(STATE_PATH)}.tmp-${process.pid}-${Date.now()}`);
