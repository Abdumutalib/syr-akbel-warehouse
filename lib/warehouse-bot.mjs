...
export function changeAdminPassword(state, oldPassword, newPassword) {
  if (!state.admin) throw new Error("Admin account topilmadi");
  if (!verifyCredentialHash(oldPassword, state.admin.password)) {
    throw new Error("Eski parol noto'g'ri");
  }
  if (!newPassword || String(newPassword).length < 4) {
    throw new Error("Yangi parol kamida 4 ta belgidan iborat bo'lishi kerak");
  }
  state.admin.password = hashCredential(newPassword);
  return true;
}
