...
export function normalizeWarehouseState(parsed = {}) {
  ...
  return {
    ...
    stock: Array.isArray(parsed.stock)
      ? parsed.stock
      : [
          { id: 'cheese', name: 'Pishloq', quantity: parsed.warehouse?.currentStockKg ?? 0 }
        ],
...
export function saveWarehouseState(dbPath, state) {
  ...
  if (serializableState.stock && serializableState.warehouse) {
    const cheese = serializableState.stock.find((item) => item.id === "cheese");
    if (cheese) {
      serializableState.warehouse.currentStockKg = cheese.quantity;
    }
  }
