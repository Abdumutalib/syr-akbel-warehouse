...
export const STAFF_PERMISSION_KEYS = ["seller", "customers", "cash", "transfer", "admin_panel", "orders", "write_cash_sale", "write_transfer_sale"];
...
function hashCredential(value) {
  ...
  const hash = crypto.pbkdf2Sync(String(value), salt, HASH_ITERATIONS, HASH_KEY_LENGTH, HASH_DIGEST).toString("hex");
  return `pbkdf2:${salt}:${hash}`;
}

function verifyCredentialHash(value, stored) {
...
function isHashedCredential(value) {
  return Boolean(value && String(value).startsWith("pbkdf2:"));
}

function migrateCredentialIfNeeded(account, field, value) {
  if (account[field] && !isHashedCredential(account[field]) && String(account[field]) === String(value)) {
    account[field] = hashCredential(value);
    return true;
  }
  return false;
}
...
function calculateTransactionTotalPrice(entry, pricing, user = null) {
  ...
  export function summarizeOperatorDailyActivity(state, operator, options = {}) {
    ...
    const summary = approvedTransactions.reduce((result, entry) => {
      ...
      if (((entry.kind || "sale") === "sale" || entry.kind === "return") && Number(entry.amountKg || 0) > 0) {
        const isReturn = entry.kind === "return";
        const mult = isReturn ? -1 : 1;
        result.totalBlocks += blocks * mult;
        result.totalSales += totalPrice * mult;
        if (priceType === "transfer") {
          result.transferBlocks += blocks * mult;
          result.transferSales += totalPrice * mult;
        } else {
          result.cashBlocks += blocks * mult;
          result.cashSales += totalPrice * mult;
        }
      }
      return result;
    }, {
...
export function normalizeWarehouseState(parsed = {}) {
  ...
  const parsedStock = Array.isArray(parsed.stock) ? parsed.stock : [];
  const cheeseStock = parsedStock.find((item) => item?.id === "cheese") || null;
  const rawWarehouseStock = parsed.warehouse?.currentStockKg;
  const warehouseStock = rawWarehouseStock == null || rawWarehouseStock === "" ? NaN : Number(rawWarehouseStock);
  const stockFallback = cheeseStock?.quantity == null || cheeseStock?.quantity === "" ? NaN : Number(cheeseStock.quantity);
  const currentStockKg = Number.isFinite(warehouseStock)
    ? warehouseStock
    : (Number.isFinite(stockFallback) ? stockFallback : 0);
  const stockItems = parsedStock.length
    ? parsedStock.map((item) => item?.id === "cheese" ? { ...item, quantity: currentStockKg } : item)
    : [{ id: "cheese", name: "Pishloq", quantity: currentStockKg }];
  if (!stockItems.some((item) => item?.id === "cheese")) {
    stockItems.unshift({ id: "cheese", name: "Pishloq", quantity: currentStockKg });
  }
  return {
    ...initialState(),
    ...parsed,
    ...
    users: Array.isArray(parsed.users)
      ? parsed.users.map((user) => {
          ...
          const pricing = normalizeWarehousePricingInput(parsed.warehouse || {});
          const debtInfo = recalculateDebtBreakdown({ transactions, users: [] }, normalizedUser.id, pricing);
          return {
            ...normalizedUser,
            _debtCache: debtInfo,
          };
...
function sanitizeTransactionPhoto(photo) {
  ...
  const url = String(photo.url || photo.dataUrl || "").trim();
  const fileName = String(photo.fileName || originalName || id || "").trim();
...
export function loadWarehouseState(dbPath) {
  if (!fs.existsSync(dbPath)) {
    ...
    saveWarehouseState(dbPath, seed);
    return seed;
  }
  ...
  try {
    ...
    try { fs.renameSync(dbPath, `${dbPath}.broken-${Date.now()}`); } catch {}
...
export function saveWarehouseState(dbPath, state) {
  ensureParentDir(dbPath);
  const serializableState = state && typeof state === "object" ? state : {};
  if (serializableState.warehouse) {
    const currentStockKg = Number(serializableState.warehouse.currentStockKg || 0);
    if (!Array.isArray(serializableState.stock)) {
      serializableState.stock = [];
    }
    const cheese = serializableState.stock.find((item) => item?.id === "cheese");
    if (cheese) {
      cheese.quantity = currentStockKg;
    } else {
      serializableState.stock.unshift({ id: "cheese", name: "Pishloq", quantity: currentStockKg });
    }
  }
...
export function recordTelegramMessage(state, { telegramId, customerName, text, type, result }) {
  ...
  state.telegramMessages = state.telegramMessages.filter((msg) => msg.receivedAt >= twoDaysAgo);
...
export function listTelegramMessages(state, limit = 50) {
  ...
  const twoDaysAgo = new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString();
  return state.telegramMessages.filter((msg) => msg.receivedAt >= twoDaysAgo).slice(0, limit);
}
...
function normalizeCustomerProfile(user = {}) {
  ...
  return {
    ...
    telegramFirstName: String(user?.telegramFirstName || "").trim() || null,
...
export function updateWarehouseOrder(state, orderId, payload = {}, actor = null) {
  if (!Array.isArray(state.orders)) {
  ...
  if (hasOwn(payload, 'customerName') || hasOwn(payload, 'organizationName') || hasOwn(payload, 'taxId')) {
    const identity = normalizeOrderIdentity({
      customerName: hasOwn(payload, 'customerName') ? payload.customerName : order.customerName,
      organizationName: hasOwn(payload, 'organizationName') ? payload.organizationName : order.organizationName,
      taxId: hasOwn(payload, 'taxId') ? payload.taxId : order.taxId,
    });
    order.customerName = identity.customerName;
    order.organizationName = identity.organizationName;
    order.taxId = identity.taxId;
  }
  if (hasOwn(payload, 'phone')) {
    order.phone = normalizeOrderPhone(payload.phone);
  }
  if (hasOwn(payload, 'note')) {
    order.note = normalizeOrderNote(payload.note);
  }
  return sanitizeWarehouseOrder(order);
}

export function deleteWarehouseOrder(state, orderId) {
  if (!Array.isArray(state.orders)) {
    throw new Error("Zakaz topilmadi");
  }
  const idx = state.orders.findIndex((entry) => entry.id === Number(orderId));
  if (idx === -1) {
    throw new Error("Zakaz topilmadi");
  }
  const [order] = state.orders.splice(idx, 1);
  return { id: order.id, customerName: order.customerName };
}

function findOrCreateUser(state, fullName, telegramId = null, telegramUsername = null, telegramFirstName = null) {
  const normalized = normalizeName(fullName);
  ...
  if (!user) {
    ...
    if (telegramFirstName && !user.telegramFirstName) {
...
function normalizeStaffRole(value) {
  ...
  if (role !== "seller" && role !== "accountant" && role !== "admin") {
    throw new Error("Rol faqat seller, accountant yoki admin bo'lishi mumkin");
  }
  return role;
}
...
function defaultPermissionsForRole(role) {
  ...
  if (role === "seller") {
    return ["seller", "customers", "orders", "write_cash_sale", "write_transfer_sale"];
  }
  if (role === "accountant") {
    return ["cash", "transfer", "customers", "orders"];
  }
  return [];
}
...
function getAccountPermissions(account) {
  // Admin role always has full access to all permission keys
  if (String(account?.role || "").trim().toLocaleLowerCase("en-US") === "admin") {
    return [...STAFF_PERMISSION_KEYS];
  }
...
function normalizeLinkPermission(value, role) {
  ...
  if (!permission || permission === "all" || permission === "universal") {
    return "all";
  }
...
function sanitizeAccessLink(link) {
  return {
    ...
    unlockedAt: link.unlockedAt || null,
  };
}
...
export function createStaffAccount(state, payload = {}) {
  ...
  const normalizedPin = String(payload.pin || "").trim();
  const account = {
    id: nextId(state, "staff"),
    fullName,
    username,
    password: hashCredential(password),
    pin: normalizedPin ? hashCredential(normalizedPin) : null,
    role: role,
    permissions,
    accessLinks: [],
    createdAt: new Date().toISOString(),
...
export function updateStaffAccountPin(state, staffId, pin) {
  ...
  if (normalizedPin && /^(\d)\1+$/.test(normalizedPin)) {
    throw new Error("PIN bir xil raqamlardan iborat bo'lmasligi kerak (masalan, 0000 yoki 1111)");
  }
  if (normalizedPin && /^(0123|1234|2345|3456|4567|5678|6789|9876|8765|7654|6543|5432|4321|3210)\d*$/.test(normalizedPin)) {
    throw new Error("PIN juda oddiy ketma-ketlikdan iborat bo'lmasligi kerak (masalan, 1234)");
  }
  account.pin = normalizedPin ? hashCredential(normalizedPin) : null;
  return sanitizeStaffAccount(account);
}
...
export function createStaffAccessLink(state, staffId, permission) {
  ...
  const normalizedPermission = "all";
  const accountPermissions = getAccountPermissions(account);
  if (normalizedPermission !== "all" && !accountPermissions.includes(normalizedPermission)) {
    throw new Error("Xodimda bu sahifa uchun ruxsat yo'q");
  }
  if (!accountPermissions.length) {
    throw new Error("Xodimga avval kamida bitta ruxsat bering");
  }
  const link = {
...
export function authenticateStaffAccessToken(state, token, permission) {
  ...
  for (const account of Array.isArray(state.staffAccounts) ? state.staffAccounts : []) {
    ...
    if (normalizedPermission && link.permission !== "all" && link.permission !== normalizedPermission) {
      return null;
    }
    const accountPermissions = getAccountPermissions(account);
    if (normalizedPermission && !accountPermissions.includes(normalizedPermission)) {
      return null;
    }
    if (link.permission !== "all" && !accountPermissions.includes(link.permission)) {
      return null;
    }
    const isUnlocked = Boolean(link.unlockedAt) && (Date.now() - new Date(link.unlockedAt).getTime() < 365 * 24 * 60 * 60 * 1000);
    return {
...
export function authenticateStaffAccount(state, username, password, options = {}) {
  ...
  const requiredPermission =
    options && !Array.isArray(options) && options.permission
      ...
      : null;
  const account = (Array.isArray(state.staffAccounts) ? state.staffAccounts : []).find(
    (entry) => normalizeUsername(entry.username) === normalizedUsername && verifyCredentialHash(password, entry.password)
  );
  if (account) {
    migrateCredentialIfNeeded(account, "password", password);
  }
  if (!account) {
    return null;
  }
...
export function verifyStaffPin(state, token, pin) {
  const account = authenticateStaffAccessToken(state, token);
  if (!account) {
    throw new Error("Ruxsat berilmagan");
  }
  const realAccount = (Array.isArray(state.staffAccounts) ? state.staffAccounts : []).find(
    (entry) => entry.id === account.id
  );
  if (!realAccount || !realAccount.pin) {
    throw new Error("PIN o'rnatilmagan");
  }
  if (!verifyCredentialHash(String(pin || "").trim(), realAccount.pin)) {
    throw new Error("PIN noto'g'ri");
  }
  migrateCredentialIfNeeded(realAccount, "pin", String(pin || "").trim());

  // Mark this specific link as unlocked for this session
  const link = realAccount.accessLinks.find(l => l.token === token && !l.revokedAt);
  if (link) {
    link.unlockedAt = new Date().toISOString();
  }
  return true;
}
...
function recalculateDebtBreakdown(state, userId, pricingInput) {
  ...
  const totals = approved.reduce((summary, entry) => {
    ...
    if (Number(entry.amountKg || 0) > 0) {
      ...
      const mult = isReturn ? -1 : 1;
      if (priceType === "transfer") {
        summary.transferSales += totalPrice * mult;
      } else {
        summary.cashSales += totalPrice * mult;
      }
    }
  ...
  // Allow negative values (credit) when returns exceed prior sales.
  // A negative debt means the warehouse owes money back to the customer.
  const cashDebt = Math.round(totals.cashSales - totals.cashPaid);
  const transferDebt = Math.round(totals.transferSales - totals.transferPaid);
  const currentDebt = cashDebt + transferDebt;
  return {
...
function matchesPaymentType(entry, paymentType) {
  ...
  const transferPaid = Number(entry.transferPaidAmount || 0);
  const paymentAmount = paymentType === "cash" ? cashPaid : transferPaid;
  const totalPaid = cashPaid + transferPaid;
  const isSale = (entry.kind || "sale") === "sale" && Number(entry.amountKg || 0) > 0;
  if (isSale) {
    return paymentAmount > 0 || (totalPaid <= 0 && entry.priceType === paymentType);
  }
...
export function approveTransaction(state, txId, paidAmount, options = {}) {
  ...
  // Recalculate debt cache from scratch to handle cross-payments and splits correctly.
  if (user) {
    user._debtCache = recalculateDebtBreakdown(state, user.id, pricing);
  }

  return {
    transaction: tx,
    user,
    debt: user?._debtCache?.currentDebt ?? recalculateDebt(state, tx.userId, pricing),
...
export function deleteCustomer(state, userId) {
  ...
  const restoredStockKg = relatedTransactions.reduce((sum, entry) => {
    ...
    const stockRestoreMultiplier = entry.kind === "return" ? -1 : 1;
    return sum + Number(entry.amountKg || 0) * stockRestoreMultiplier;
  }, 0);
...
export function clearCustomerHistory(state, userId) {
  const customer = getUserById(state, userId);
  if (!customer) {
    throw new Error("Mijoz topilmadi");
  }
  ...
  if (!Array.isArray(state.deletedCustomers)) {
  ...
  return {
    customer: { id: customer.id, fullName: customer.fullName },
    clearedTransactions: relatedTransactions.length,
    restoredStockKg,
  };
}
...
export function permanentlyDeleteCustomer(state, customerId) {
  const index = (state.deletedCustomers || []).findIndex((entry) => Number(entry.customer?.id) === Number(customerId));
  if (index === -1) {
    throw new Error("Mijoz o'chirilganlar ro'yxatida topilmadi");
  }
  const archive = state.deletedCustomers[index];
  state.deletedCustomers.splice(index, 1);
  return {
    ...
    fullName: archive.customer?.fullName || "Noma'lum",
  };
}
...
export function recordApprovedSale(state, payload = {}, options = {}) {
  ...
  // Always recalculate from scratch to handle cross-payments correctly
  if (result.user) {
    result.user._debtCache = recalculateDebtBreakdown(state, result.user.id, pricing);
  }
  const debt = result.user?._debtCache?.currentDebt ?? recalculateDebt(state, result.user.id, pricing);
  return {
    ...result,
    debt,
...
export function recordCustomerPayment(state, payload = {}, options = {}) {
  const pricing = resolveWarehousePricing(options.pricing ?? options.pricePerKg, state);
  const paymentPreview = normalizePaymentBreakdown(payload);
  if (paymentPreview.cashPaidAmount + paymentPreview.transferPaidAmount <= 0) {
...
export function recordCustomerReturn(state, payload = {}, options = {}) {
  const pricing = resolveWarehousePricing(options.pricing ?? options.pricePerKg, state);
  const amountKg = Number(payload.amountKg || 0);
  if (!Number.isFinite(amountKg) || amountKg <= 0) {
    throw new Error("Qaytarilgan kg noto'g'ri");
  }
  const blockCount = payload.blockCount == null || payload.blockCount === ""
    ? 0
    : normalizeBlockCount(payload.blockCount, "Qaytarilgan blok soni noto'g'ri");
  const result = createApprovedTransaction(
    state,
    {
      ...payload,
...
export function listCustomerSummaries(state, pricingInput) {
  ...
  return state.users
    .map((user) => {
      ...
      const totalTakenKg = approved.reduce((sum, entry) => {
        const isReturn = entry.kind === "return";
        return sum + (isReturn ? -Number(entry.amountKg || 0) : Number(entry.amountKg || 0));
      }, 0);
      const debtSummary = recalculateDebtBreakdown(state, user.id, pricing);
      const cashSales = user._debtCache?.cashSales ?? debtSummary.cashSales;
      const transferSales = user._debtCache?.transferSales ?? debtSummary.transferSales;
      const cashPaid = user._debtCache?.cashPaid ?? debtSummary.cashPaid;
      const transferPaid = user._debtCache?.transferPaid ?? debtSummary.transferPaid;
      return {
        id: user.id,
        ...
        cashPaid,
        transferPaid,
        totalPaid: cashPaid + transferPaid,
        pendingCount: pending.length,
        cashDebt: user._debtCache?.cashDebt ?? debtSummary.cashDebt,
        transferDebt: user._debtCache?.transferDebt ?? debtSummary.transferDebt,
        currentDebt: user._debtCache?.currentDebt ?? debtSummary.currentDebt,
      };
    })
...
function buildCustomerHistoryEntry(entry, pricing, user) {
  ...
  return {
    ...
    paidAmount: Number(entry.paidAmount != null ? entry.paidAmount : (Number(entry.cashPaidAmount || 0) + Number(entry.transferPaidAmount || 0))),
...
export function recordWarehouseReceipt(state, payload = {}) {
  ...
  const kind = payload.kind === "return" ? "return" : "receipt";
  const receipt = {
...
export function summarizeWarehouseReceipts(receipts = []) {
  return (Array.isArray(receipts) ? receipts : []).reduce(
    (summary, entry) => {
      const isReturn = entry.kind === "return";
      const mult = isReturn ? -1 : 1;
      summary.totalKg += Number(entry.amountKg || 0) * mult;
      summary.totalBlocks += Number(entry.blockCount || 0) * mult;
      summary.totalPrice += Number(entry.totalPrice || 0) * mult;
      return summary;
    },
    {
      totalKg: 0,
...
export function updateWarehousePricing(state, payload = {}) {
  ...
  if (Array.isArray(state.users)) {
    for (const user of state.users) {
      user._debtCache = recalculateDebtBreakdown(state, user.id, nextPricing);
    }
  }
  return nextPricing;
}
