...
export function approveTransaction(state, txId, paidAmount, options = {}) {
  ...
  // Update incremental debt cache
  if (user && user._debtCache) {
    const isTransfer = tx.priceType === "transfer";
    if (Number(tx.amountKg || 0) > 0) {
      if (isTransfer) user._debtCache.transferDebt += tx.totalPrice;
      else user._debtCache.cashDebt += tx.totalPrice;
    }
    if (isTransfer) user._debtCache.transferDebt -= tx.paidAmount;
    else user._debtCache.cashDebt -= tx.paidAmount;
    user._debtCache.cashDebt = Math.max(0, user._debtCache.cashDebt);
    user._debtCache.transferDebt = Math.max(0, user._debtCache.transferDebt);
    user._debtCache.currentDebt = user._debtCache.cashDebt + user._debtCache.transferDebt;
  }
