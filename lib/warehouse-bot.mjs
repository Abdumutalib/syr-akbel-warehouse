...
export function extractNameAndAmount(text) {
  ...
  const nameMatch = input.match(/^([\p{Script=Cyrillic}\s]+?)\s*(?:\d+(?:[.,]\d+)?\s*(?:kg|кг|кило))/iu);
  let name = "Номаълум";
  if (nameMatch?.[1]) {
    name = nameMatch[1].trim().replace(/(га|ка|нинг|дан)$/iu, "").trim();
  } else {
    const afterMatch = input.match(/(?:kg|кг|кило)\s+([\p{Script=Cyrillic}\s]+)/iu);
    if (afterMatch?.[1]) {
...
function getOrCreateUserFromPayload(state, payload = {}) {
...
function createApprovedTransaction(state, payload, options = {}) {
  const user = getOrCreateUserFromPayload(state, payload);
...
export function upsertCustomer(state, payload = {}) {
  const user = getOrCreateUserFromPayload(state, payload);
  return user;
}
