...
function recalculateDebtBreakdown(state, userId, pricingInput) {
  ...
  // Allow cross-payment offsets: if a customer overpays in one category,
  // the surplus automatically covers debt in the other category.
  let rawCashDebt = totals.cashSales - totals.cashPaid;
  let rawTransferDebt = totals.transferSales - totals.transferPaid;

  const user = state.users.find((candidate) => candidate.id === userId) || null;
  const cats = user?.paymentCategories || [];
  if (cats.length === 1) {
    if (rawTransferDebt < 0 && rawCashDebt > 0) {
      rawCashDebt += rawTransferDebt;
      rawTransferDebt = 0;
    } else if (rawCashDebt < 0 && rawTransferDebt > 0) {
      rawTransferDebt += rawCashDebt;
      rawCashDebt = 0;
    }
  }

  const cashDebt = Math.max(0, Math.round(rawCashDebt));
  const transferDebt = Math.max(0, Math.round(rawTransferDebt));
  return {
    cashDebt,
    transferDebt,
